...
public class Tokenizer {
	public static List<LiteralToken> tokenize(LiteralToken depar) {
		return tokenizeFully(depar).stream().filter(LiteralToken::isMeaningful)
	...
	public static List<LiteralToken> tokenizeFully(LiteralToken parentToken) {
		...
		loop: for (int i = 0; i < parentToken.token.length(); i++) {
			switch (parentToken.token.charAt(i)) {
				case '[':
					...
					if (closebracket < 0) {
						...
						tokens.add(LiteralToken
								.errorToken(parentToken.token.substring(i)));
						return tokens;
					}
					// don't just ignore everything between brackets,
					// add it! (It'll be filtered out for everything but
					// syntax highlighting purposes anyway)
				...
				case '\'':
				case '\"':
					char prev = parentToken.token.charAt(i - 1);
