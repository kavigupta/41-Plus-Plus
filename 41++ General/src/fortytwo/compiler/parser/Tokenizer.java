...
public class Tokenizer {
	 ...
	 * @return
	 */
	public static List<LiteralToken> tokenizeFully(LiteralToken parentToken) {
		...
		loop: for (int i = 0; i < parentToken.token.length(); i++) {
			switch (parentToken.token.charAt(i)) {
				case '[':
					...
					toklen = 0;
					int closebracket = findCloseBracket(parentToken.token,
							i);
					if (closebracket < 0) {
					...
					tokens.add(parentToken.subToken(i, closebracket + 1));
					i = closebracket;
					continue loop;
				case '(':
					add(parentToken, i, toklen, tokens);
				...
				case ')':
				case ']':
					tokens.add(LiteralToken.errorToken(parentToken.token
							.substring(i)));
				...
				case '.':
					if (i + 1 < parentToken.token.length()
							&& !Character.isDigit(parentToken.token
									.charAt(i + 1))) {
						add(parentToken, i, toklen, tokens);
						toklen = 0;
						tokens.add(parentToken.subToken(i, i + 1));
				...
				case '\"':
					if (i - 1 < 0
							|| Language.isTerminator(parentToken.token
									.charAt(i - 1))) {
						add(parentToken, i, toklen, tokens);
						toklen = 0;
						...
						tokens.add(parentToken
								.subToken(i, closequote + 1).unescape());
						i = closequote;
						continue loop;
					}
					break;
			...
			if (i - 1 >= 0
					&& Character.isDigit(parentToken.token.charAt(i - 1))
