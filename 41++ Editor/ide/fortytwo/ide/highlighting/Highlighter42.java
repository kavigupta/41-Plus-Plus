...
public class Highlighter42 extends AbstractTokenMaker {
		...
		public Token getTokenList(Segment text, int startTokenType,
			...
			addAllTokens(
					Tokenizer.tokenizeFully(LiteralToken.entire(code)).stream()
		...
		private void addAllTokens(List<LiteralToken> tokens, Segment text,
			...
			for (int i = 0; i < tokens.size(); i++) {
				...
				if (tok.token.startsWith("(")) {
					...
					addAllTokens(Tokenizer.tokenizeFully(depar), text,
								startOffset);
		...
		public void addToken(List<LiteralToken> tokens, int index, Segment text,
				int startOffset) {
			int currentTokenStart = tokens.get(index).context.start
					+ text.offset;
		...
		public void addErrorToken(LiteralToken error, Segment text,
				int startOffset, int tokenStart) {
		...
		private static String previousWord(List<LiteralToken> tokens,
				int index) {
